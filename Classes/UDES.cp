...
#define UTInt64GetHi(_i) ((KUInt32) (_i >> 32))
#define UTInt64GetLo(_i) ((KUInt32) (_i))
#define UTInt64Set(_i,_hi,_lo) *(_i) = ((((KUInt64) _hi) << 32) | _lo)
#define KDEBUG1
#define KDEBUG2
// -------------------------------------------------------------------------- //
...
UDES::KeySchedule( const KUInt64& inKey, SSubKey outSubKeys[16] )
{
	...
	KUInt32 inKeyHigh = UTInt64GetHi( inKey );
	KUInt32 inKeyLow = UTInt64GetLo( inKey );
...
UDES::NewtonKeySchedule( const KUInt64& inKey, SSubKey outSubKeys[16] )
{
	...
	KUInt32 theKeyHigh = UTInt64GetHi( inKey );
	KUInt32 theKeyLow = UTInt64GetLo( inKey );
	KUInt64 theDESKey;
	UTInt64Set( &theDESKey, theKeyHigh << 1, theKeyLow << 1 );
...
UDES::EncodeBlock( const SSubKey inSubKeys[16], KUInt64* ioData )
{
	...
	// (cf l'algorithme).
	KUInt32 ioDataLeft = UTInt64GetHi( *ioData );
	KUInt32 ioDataRight = UTInt64GetLo( *ioData );
	...
	UTInt64Set( ioData, ioDataLeft, ioDataRight );
}
			...
			KUInt8* outData )
{
	...
	while (theIndex < inSize)
	{
		...
		if ((theIndex % 4) == 0)
		{
			...
			KUInt32 theFirst4Bytes = UTInt64GetHi( theWord );
			KUInt32 theLast4Bytes = UTInt64GetLo( theWord );
			...
			bool inNewtonCompat )
{
	...
	do
	{
		...
		// Character #1
		theUniChar = OSSwapBigToHostInt16( inString[ indexStr++ ] );
		if (theUniChar)
		{
		...
		KUInt64 tmpKey;
        UTInt64Set(&tmpKey, tmpKeyHi, tmpKeyLo);
		...
		tmpKeyHi = UTInt64GetHi( tmpKey );
		tmpKeyLo = UTInt64GetLo( tmpKey );
